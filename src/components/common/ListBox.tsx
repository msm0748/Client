...
interface Props {
  data: IListBox[];
  selected: IListBox;
  setSelected: React.Dispatch<React.SetStateAction<IListBox | undefined>>;
}
      ...
      <HeadlessListBox value={selected} onChange={setSelected}>
        <div className="relative mt-1">
          ...
          <Transition
            ...
            leaveTo="opacity-0"
          >
            <HeadlessListBox.Options className="z-10 absolute mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black/5 focus:outline-none sm:text-sm">
              {data.map((item) => (
